...
export const renderSnapshotContents = (contents: SnapshotContents, editable?: EditContext): string => {
  ...
  const rows = contents.flags
    .map((flag) => {
      const editCell = editable === undefined ? '' : `<td>${renderFeatureEditForm(flag, editable)}</td>`;
      return `<tr><td>${escapeHtml(flag.key)}</td><td>${flag.type}</td><td>${String(flag.enabled)}</td><td><code>${escapeHtml(JSON.stringify(flag.defaultValue))}</code></td><td>${String(flag.ruleCount)}</td>${editCell}</tr>`;
    })
    .join('\n');
  return `<table>
<thead><tr><th>Flag</th><th>Type</th><th>Enabled</th><th>Default</th><th>Rules</th>${editable === undefined ? '' : '<th>Edit</th>'}</tr></thead>
<tbody>
${rows}
</tbody>
</table>`;
